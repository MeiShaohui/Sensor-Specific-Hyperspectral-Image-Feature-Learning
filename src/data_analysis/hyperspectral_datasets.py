...
class DataSet:
    ...
    def get_data(self):
        data_file = os.path.expanduser("{}/{}".format(self.dir, self.data_file))
        assert os.path.exists(data_file)
        self.data = sio.loadmat(data_file)[self.data_key]

    def get_labels(self):
        label_file = os.path.expanduser("{}/{}".format(self.dir, self.label_file))
        assert os.path.exists(label_file)
...
class HSIDataSet(DataSet, HSIDataSetInfo):
    def __init__(self, name, hsi_dir='../../hyperspectral_datas/'):
        self.HSI_dir = os.path.expanduser(hsi_dir)
